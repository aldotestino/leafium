...
describe('Leafium unit test', () => {
  ...
  it('allows to remove a gateway', async () => {
    await leafium.addGateway('eui-8989898989', 'lucky lizard', '41.16092034214199', '16.4141807908589', 20);
    await leafium.removeGateway('eui-8989898989');
    const myGateways = await leafium.getMyGateways();
    expect(myGateways).to.be.empty;
  });

  it('doesn\'t allow to remove a gateway if it\'s not of the user ', async () => {
    await leafium.addGateway('eui-8989898989', 'lucky lizard', '41.16092034214199', '16.4141807908589', 20);
    const leafiumUser1 = await leafium.connect(user1);
    await expect(leafiumUser1.removeGateway('eui-8989898989')).to.be.revertedWith('Gateway not found!');
  });
  ...
  it('doesn\'t give tokens to the user if the gateway it\'s not his', async () => {
    await leafium.addGateway('eui-8989898989', 'lucky lizard', '41.16092034214199', '16.4141807908589', 20);
